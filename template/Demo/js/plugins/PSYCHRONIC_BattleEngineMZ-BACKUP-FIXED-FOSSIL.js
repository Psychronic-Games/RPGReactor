...
Window_ActorCommand.prototype.addEscapeCommand = function() {
    ...
    Game_Action.prototype.apply = function(target) {
        ...
        if (wasDead && !target.isDead() && target._sprite) {
            ...
            sprite.opacity = 255;
            sprite.visible = true;
            if (sprite.refreshMotion) sprite.refreshMotion();  // Reset to appropriate motion (e.g., 'wait')
        }
    };
    ...
    function canApplyActionToTarget(action, target) {
        ...
        console.log(`🔍 Checking target validation: ${subject.name()} (${subject.isActor() ? 'Actor' : 'Enemy'}) -> ${target.name()} (${target.isActor() ? 'Actor' : 'Enemy'}), Scope: ${scope}`);
        ...
        if (target.isAlive()) {
            ...
            switch (scope) {
                ...
                case 3: // One Random Enemy
                    ...
                    console.log(`⚔️ Enemy scope - Subject is ${subject.isActor() ? 'Actor' : 'Enemy'}, Target is ${target.isActor() ? 'Actor' : 'Enemy'}: ${result}`);
                    break;
 ...
 showDamage = true
            ...
            if (playAnimation) {
                ...
                if (animationId > 0 && targets.length > 0) {
                    console.log(`🎬 Playing animation ${animationId} on ${targets.length} targets`);
    ...
    Sprite_StateOverlay.prototype.animationWait = function() {
        ...
        // Default is 8 frames, so 8/speed gives us the multiplier effect
    ...
    class Sprite_Projectile extends Sprite {
        ...
        update() {
            ...
            if (this.throwData.spin !== 0) {
                this.rotation += (this.throwData.spin * Math.PI / 180) * (1/60); // Convert degrees to radians per frame
            }
        }
 ...
 idle: { index: 18, loop: false }  // New idle motion with non-looping behavior
    ...
    Sprite_Enemy.prototype.startMove = function(x, y, duration) {
        ...
        if (this._targetOffsetX !== x || this._targetOffsetY !== y) {
            ...
            console.log(`🎯 Enemy ${this._enemy?.name()} starting move with offset (${x}, ${y}) for ${duration} frames.`);
        }
    };
    ...
    Sprite_Battler.prototype.setupDamagePopup = function() {
        ...
        if (this._damages && this._damages.length > 0) {
            ...
            if (latestDamageSprite) {
                latestDamageSprite.zIndex = 10000; // High value to ensure it's above enemy sprites, icons, projectiles, etc.
            }
        }
    };
    ...
    Sprite_Actor.prototype.setActorHome = function(index) {
        ...
        // Fall back to default positioning if custom is disabled or no position found
        _Sprite_Actor_setActorHome.call(this, index);
    };

    // FIXED: Override updateTargetPosition to prevent constant stepForward calls for charset battlers
    ...
    Sprite_Actor.prototype.updateTargetPosition = function() {
        ...
        // Use default behavior for SV battlers
    ...
    class Sprite_ActionIcon extends Sprite {
        ...
        setupPosition() {
            ...
            if (battlerSprite) {
                ...
                console.log(`🎯 Icon z-index: ${this.zIndex} (battler: ${this.battler.isActor() ? 'actor' : 'enemy'}, layer: ${layer})`);
            }
        }
        ...
        updateIconData(newIconData) {
            ...
            if (oldZIndex !== this.zIndex) {
                console.log(`Z-index changed from ${oldZIndex} to ${this.zIndex}, repositioning in container`);
                this.repositionInContainer();
            }

            console.log(`Updated icon ${this.iconIndex} - Position: (${this.iconData.x}, ${this.iconData.y}), Z-Index: ${this.zIndex}, Layer: ${this.iconData.layer || 'normal'}`);
        }
        ...
        repositionInContainer() {
            ...
            // Debug: Log current container order
            this.debugContainerOrder();
        }
        ...
        debugContainerOrder() {
            ...
            console.log(`🔍 Container order after repositioning:`);
        ...
        update() {
            ...
            if (battlerSprite) {
                ...
                // Update Z-index when Y position changes
                const layer = ZIndexManager.normalizeLayer(this.iconData.layer);
                ZIndexManager.updateSpriteZIndex(this, 'icon', layer);
            }
    ...
    function addBattlerIcon(battler, iconData) {
        ...
        if (battler._actionIcons.has(iconData.index)) {
            ...
            console.log(`🔄 Updated existing icon ${iconData.iconType} (index ${iconData.index})`);
            return;
        }
        ...
        if (battleField) {
            ...
            console.log(`✅ Added icon ${iconData.iconType} at position ${insertIndex} with z-index ${iconSprite.zIndex}`);
    ...
    function removeBattlerIcon(battler, index) {
        ...
        if (battler._actionIcons && battler._actionIcons instanceof Map && battler._actionIcons.has(index)) {
            ...
            console.log(`✅ Removed icon index ${index} from ${battler.name()}`);
        }
    }
    ...
    function clearAllBattlerIcons(battler) {
        ...
        if (battler._actionIcons && battler._actionIcons instanceof Map) {
            ...
            battler._actionIcons.clear();
            console.log(`✅ Cleared all icons from ${battler.name()}`);
        }
    }
    ...
    class CustomSequenceStep extends ActionStep {
                    ...
                    $battleTiming.executeWithDelay(() => {
                        ...
                        for (const balloonTarget of balloonTargets) {
                            if (balloonTarget && balloonTarget._sprite) {
                                console.log(`🎈 Requesting balloon ${command.id} for ${balloonTarget.name()}`);
        ...
        executeThrowCommand(command, callback) {
            ...
            console.log(`🎯 Fired projectile '${command.projectile}' at ${targets.length} targets using '${targetMode}' mode`);
        }
    ...
    class ThrowProjectileStep extends ActionStep {
        ...
        perform() {
            ...
            const subjectSprite = this.subject._sprite;
            if (!subjectSprite) {
                console.log("No subject sprite for projectile");
                this.complete();
                return;
            }
        ...
        launchProjectiles(subjectSprite, targetPositions) {
            ...
            for (const targetPos of targetPositions) {
                ...
                console.log(`🎯 Launched projectile from (${subjectSprite.x}, ${subjectSprite.y}) to (${targetPos.x}, ${targetPos.y})`);
            }
        }
    }
    ...
    Spriteset_Battle.prototype.createEnemies = function() {
        ...
        this._enemySprites.forEach((sprite, index) => {
            ...
            // Ensure home position is set based on enemy's screen position
            if (sprite._enemy) {
                ...
                console.log(`🏠 Enemy ${sprite._enemy.name()} initialized at home: (${homeX}, ${homeY})`);
            }
        });
    };
    ...
    Spriteset_Battle.prototype.createActors = function() {
        ...
        this._actorSprites.forEach(sprite => {
            ...
            // Position actors off-screen for entry animation
            if (sprite._homeX !== undefined && sprite._homeY !== undefined) {
                // Store the actual target home position
                sprite._targetHomeX = sprite._homeX;
                sprite._targetHomeY = sprite._homeY;

                // Position off-screen based on orientation
                if (battleOrientation === 'Vertical') {
                    // Start at bottom of screen
                    sprite._homeX = sprite._targetHomeX;
                    sprite._homeY = Graphics.height + 100;
                } else {
                    // Start at right of screen
                    sprite._homeX = Graphics.width + 100;
                    sprite._homeY = sprite._targetHomeY;
                }
    ...
    BattleManager.endAction = function() {
        console.log("🎬 BattleManager.endAction - Phase:", this._phase);
        ...
        if (this._battleField && this._battleField.children) {
            ...
            if (this._sortCheckCounter >= 60) {
                ...
                for (let i = 1; i < children.length; i++) {
                    ...
                    if (prevZ > currZ) {
                        needsSort = true;
                        console.log(`🔍 Container order issue detected: ${children[i-1].constructor.name} (z=${prevZ}) before ${children[i].constructor.name} (z=${currZ})`);
                        break;
                    }
                }
            }
    ...
    BattleManager.shouldUseActionSequence = function(action) {
        ...
        for (const seqType of sequenceTypes) {
            ...
            if (customSequence && customSequence.length > 0) {
                console.log(`🎯 Found custom ${seqType} sequence for ${action.item().name}`);
                return true;
            }
        }
    ...
    BattleManager.buildCustomActionSequence = function(action, subject, targets) {
        ...
        console.log(`🎬 Building optimized custom sequence for ${itemName} with ${sequenceTypes.length} phases`);
    ...
    BattleManager.actionSequenceFinished = function() {
        console.log("🎯 Action sequence finished, clearing flag");
    ...
    BattleManager.triggerEnemyEntrySequences = function() {
        ...
        for (const enemy of enemies) {
            // Find an <action sequence: entry> tag in the ENEMY's notebox
            const entrySequence = this.getCustomSequence(enemy, 'entry');
            if (entrySequence && entrySequence.length > 0) {
                console.log(`🎬 Triggering entry sequence for ${enemy.name()}`);
    ...
    BattleManager.triggerEntrySequences = function() {
        ...
        for (let i = 0; i < actors.length; i++) {
            ...
            if (entrySequence && entrySequence.length > 0) {
                console.log(`🎬 Triggering entry sequence for ${actor.name()}`);
                this.executeCustomSequence(actor, entrySequence, 'entry');
            } else {
                // Default entry animation
                if (sprite._targetHomeX && sprite._targetHomeY) {
                    const targetOffsetX = sprite._targetHomeX - sprite._homeX;
                    const targetOffsetY = sprite._targetHomeY - sprite._homeY;
                    const delay = i * 100;

                    setTimeout(() => {
                        if (sprite.startMotion) sprite.startMotion('walk');
                        sprite.startMove(targetOffsetX, targetOffsetY, 40);
    ...
    BattleManager.executeCustomSequence = function(battler, commands, sequenceType, isLooping = false, defaultTargets = []) {
        // Allow these sequence types to run in parallel
        if (sequenceType === 'damage' || sequenceType === 'collapse' || sequenceType === 'entry' || sequenceType === 'victory' || sequenceType === 'movement') {
        ...
        // Use original method for other sequence types
