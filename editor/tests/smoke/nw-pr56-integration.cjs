...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'rr-pr56-native-'));
...
(async () => {
    try {
        ...
        const result = await driver.executeAsync(`const done=arguments[arguments.length-1];(async()=>{
            ...
            dm.data.enemies[1].actions=[{...action(1,5),conditionType:1,conditionParam1:0,conditionParam2:0}];
            const turnBaseline=authored();
            plugins=[];
            panel=await show();panel.open=true;
            let turn=panel.querySelector('[data-forecast="turn"]');
            check('engine turn control starts at one',turn.min==='1');
            check('engine warns that a turn-zero action is unreachable',!!panel.querySelector('.enemy-forecast-dead'));
            turn.value='0';turn.dispatchEvent(new Event('change',{bubbles:true}));
            check('engine clamps an entered zero to one',turn.value==='1');
            const onSpotNote='Battle AI counts turns on the spot, so a turn condition can sit one turn earlier than it reads.';
            check('engine omits the on-the-spot note',!panel.textContent.includes(onSpotNote));
            plugins=[{name:'VisuMZ_3_BattleAI',status:true,parameters:{'General:struct':JSON.stringify({
                'EnemyStyleAI:str':'classic','EnemyRatingVariance:num':'0','OnSpotAI:eval':'true'
            })}}];
            db.cleanupDatabaseDetail();panel=await show();panel.open=true;
            turn=panel.querySelector('[data-forecast="turn"]');
            check('on-the-spot turn control includes zero',turn.min==='0');
            check('on-the-spot suppresses the false unreachable warning',!panel.querySelector('.enemy-forecast-dead'));
            check('on-the-spot explains turn uncertainty',panel.textContent.includes(onSpotNote));
            turn.value='0';turn.dispatchEvent(new Event('change',{bubbles:true}));
            check('turn zero selects the gated action',turn.value==='0'&&panel.querySelectorAll('.enemy-forecast-pool-list li').length===1);
            turn.value='-2';turn.dispatchEvent(new Event('change',{bubbles:true}));
            check('on-the-spot clamps negative turns to zero',turn.value==='0');
            I18n.setLanguage('zh-Hans',{persist:false});
            db.cleanupDatabaseDetail();panel=await show();
            check('Chinese includes the incoming turn note',panel.textContent.includes(I18n.tText(onSpotNote))&&!panel.textContent.includes(onSpotNote));
            check('Chinese community corrections survive integration',I18n.tText('Critical')==='暴击'&&I18n.tText('Rates')==='抗性');
            check('turn exploration preserves authored actions and skills',authored()===turnBaseline);
