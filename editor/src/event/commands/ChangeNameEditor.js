/**
 * ChangeNameEditor - Editor for Change Name event command (code 320)
 */
class ChangeNameEditor {
    ...
    createModal() {
        ...
        this.modal.className = 'change-name-editor-modal';
        ...
        container.className = 'change-name-container';
    ...
    renderContent() {
        ...
        const container = this.modal.querySelector('.change-name-container');
        container.innerHTML = '';
        ...
        `;
        header.innerHTML = `
            <h3 style="margin: 0; color: var(--color-text-strong); font-size: 16px;">${tt('Change Name')}</h3>
        ...
        nameRow.style.cssText = 'display: flex; align-items: center; gap: 8px; padding-top: 8px; border-top: 1px solid var(--color-border);';

        const nameLabel = document.createElement('span');
        nameLabel.textContent = tt('Name:');
        nameLabel.style.cssText = 'color: var(--color-text); font-size: 13px; min-width: 100px;';
        ...
        nameInput.addEventListener('input', (e) => { this.name = e.target.value; });
    ...
    buildCommand() {
        return {
            code: 320,
            indent: 0,
            parameters: [this.actorId, this.name]
        };
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ChangeNameEditor;
}
