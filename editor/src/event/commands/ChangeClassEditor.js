/**
 * ChangeClassEditor - Editor for Change Class event command (code 321)
 */
class ChangeClassEditor {
    constructor(databaseManager, projectController) {
        ...
        // Parameters: [actorId, classId, keepEXP]
        this.actorId = 1;
        this.classId = 1;
        this.keepEXP = false;
    }
    ...
    show(command, callback) {
        ...
        if (command && command.code === 321) {
            const params = command.parameters;
            this.actorId = params[0] || 1;
            this.classId = params[1] || 1;
            this.keepEXP = params[2] !== undefined ? params[2] : false;
        } else {
            this.actorId = 1;
            this.classId = 1;
            this.keepEXP = false;
    ...
    createModal() {
        ...
        this.modal.className = 'change-class-editor-modal';
        ...
        container.className = 'change-class-container rr-modal';
    ...
    renderContent() {
        ...
        header.innerHTML = `
            <div class="rr-modal-title">${tt('Change Class')}</div>
        ...
        classSelect.style.cssText = 'padding:6px 10px; background-color:var(--color-bg-input); color:var(--color-text); border:1px solid var(--color-border-input); border-radius:3px; font-size:12px; flex:1;';

        const classes = this.databaseManager.data.classes || [];
        ...
        classSelect.addEventListener('change', (e) => { this.classId = parseInt(e.target.value); });
        ...
        // Keep EXP checkbox
        const keepExpRow = document.createElement('div');
        keepExpRow.style.cssText = 'display: flex; align-items: center; gap: 8px; padding-top: 8px; border-top: 1px solid var(--color-border);';

        const keepExpCheckbox = document.createElement('input');
        keepExpCheckbox.type = 'checkbox';
        keepExpCheckbox.id = 'keep-exp-321';
        keepExpCheckbox.checked = this.keepEXP;
        keepExpCheckbox.addEventListener('change', (e) => {
            this.keepEXP = e.target.checked;
        });
        ...
        keepExpRow.appendChild(keepExpLabel);
        content.appendChild(keepExpRow);
    ...
    buildCommand() {
        return {
            code: 321,
            indent: 0,
            parameters: [this.actorId, this.classId, this.keepEXP]
        };
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ChangeClassEditor;
}
