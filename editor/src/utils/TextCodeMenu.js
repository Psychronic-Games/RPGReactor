...
(function (root) {
    ...
    // Community-reviewed reference prose. Other locales retain the original help.
    ...
    const referenceDetail = detail => {
        const source = detail || '';
        return root.I18n?.currentLanguage() === 'zh-Hans'
            ? (TEXT_DETAIL_ZH[source] || source)
            : source;
    };
            ...
            for (const group of groups) {
                const codes = needle
                    ? group.codes.filter(code =>
                        ...
                        referenceDetail(code.detail).toLowerCase().includes(needle) ||
                ...
                for (const code of codes) {
                    ...
                    detail.textContent = referenceDetail(code.detail);
