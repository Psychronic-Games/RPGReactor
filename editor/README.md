...
| **User Interfaces** | Lay out menus, dialogs, and panels on a canvas at the game's screen size: Box, Image, Text, and Button nodes with anchors, nesting, snap and grid, undo, and buttons wired to common events, stock scenes, plugin commands, switches, variables, scripts, or other interfaces. A project opens on generated baselines of its Title Screen, Main Menu, and Game End; **Capture from Game** runs the project and lays the live scene, plugins included, under the canvas as a reference with every window listed and Add Box to start from one. Text reads escape codes plus party codes (`\GOLD`, `\PLV[n]`, `\PHP[n]`…), images can show a party member's face, and Playtest Interface opens the record alone in the game. Records live in `data/UserInterfaces.json`, invisible to RPG Maker; the stock scenes stay in use until an interface is called |
