...
const built = SP.build(plan, X0, Y0, firstId, group);
const all = kept.concat(built);
E.restorePieces(map, all);
E.setStructure(map, { group, plan: path.basename(planFile), x: X0, y: Y0, rot: 0, scale: 1 });
...
const report = SP.validate(plan, E.pieces(map), X0, Y0, map.width, map.height, Reactor3D);
