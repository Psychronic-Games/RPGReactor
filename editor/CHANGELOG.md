...
- All critical project writes (`project.rpgreactor`, `MapInfos.json`, `Map###.json`, database JSON, `System.json`, plugin manifests) go through an atomic temp-file + rename helper (`_writeFileAtomic`, falling back to plain writes for fs shims without `renameSync`) — a crash/kill/ENOSPC mid-write can no longer truncate the previous good file.
