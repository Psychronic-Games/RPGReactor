...
class AudioPlayer {
    ...
    selectAudioTrack(trackName, trackType) {
        ...
        // Load new track
        currentChannel.audio.src = track.path;
    ...
    playExternal(filePath, params = {}) {
        ...
        const isSameFile = (channel.audio.src === filePath) ||
                          (channel.currentTrack && channel.currentTrack.path === filePath);

        if (!isSameFile) {
            // Different file - load it and start from beginning
            channel.audio.pause();
            channel.audio.currentTime = 0;
            channel.audio.src = filePath;
