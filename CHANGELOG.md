...
- Added reproducible release-candidate and publication workflows for Linux x64, Windows x64, Intel macOS, and Web. Public builds pin NW.js 0.107.0 to trusted SHA-256 values, generate a clean starter, bind artifacts to their source commit and hashes, require native signing/notarization for publishable Windows/macOS candidates, and publish the inspected candidate bytes to GitHub and optional itch.io channels without rebuilding.
- Security and project lifecycle hardening escapes project-authored content on privileged editor surfaces, disables project JavaScript Character Generator parts until explicitly trusted per project, rejects unsafe/non-empty project destinations, uses token-owned exclusive project locks, ignores stale asynchronous map loads, and strengthens atomic writes against temp-file collisions and symlink replacement.
...
- Release provenance: removed the unverified bundled character style and its derived analysis/adapters/content, making Psychronic the sole built-in style. Project PNG styles remain available, while project JavaScript parts are disabled by default and run only after explicit per-project trust. Removed the unverified water-simulation animation and its Effekseer counterpart. The resulting registries contain 75 Animation Generator recipes and 105 Effekseer recipes (Energy: 14).
- Licensing documentation now scopes the repository MIT grant to RPG Reactor-owned code. `THIRD_PARTY_NOTICES.md` records identified bundled components and supplies complete pako and stb terms for header-stripped runtime bundles; third-party components retain their own licenses.
...
Current 0.95.0 validation: **343 passing tests and no failures**.
