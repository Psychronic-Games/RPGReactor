...
- [Custom user interfaces design](docs/DESIGN-USER-INTERFACES.md): the User Interfaces database section, its runtime, how it stays invisible to RPG Maker, what is built (nodes, capture from game, stock baselines) and what is next (lists, gauges, overlays).
...
- **Authorable HD-2D maps**: maps can opt into a perspective 3D presentation while retaining RPG Maker's grid, passability, events, and data format. Tileset classes and map-painted object groupings stand walls, roofs, panels, foliage, events, lights, and parallax-backed ground in the editor and runtime. Events, actors, enemies, weapons, armor, and items can carry GLB/OBJ/FBX models with in-editor rigging, carved parts, and authored animations, stored in `.r3d.json` sidecars beside the MZ data.
- **Custom user interfaces**: a User Interfaces database section lays out menus, dialogs, and panels on a canvas at the game's screen size, with boxes, images, text, and buttons wired to common events, stock scenes, plugin commands, switches, variables, or other interfaces. A project opens the section on editable baselines of its own Title Screen, Main Menu, and Game End, generated from its settings and terms; **Capture from Game** runs the project and lays the live scene, plugins and all, under the canvas as a reference with every window listed. Text nodes read escape codes plus party codes (`\GOLD`, `\PLV[n]`, `\PHP[n]`…) and images can show a party member's face. The runtime draws interfaces through the engine's own windows, the stock scenes stay in use until an interface is called, and a Reactor-tab event command opens them. See the [design](docs/DESIGN-USER-INTERFACES.md) for what is built and what is next.
