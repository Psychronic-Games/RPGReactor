...
- [Custom user interfaces design](docs/DESIGN-USER-INTERFACES.md): the plan for a User Interfaces database section, its runtime, and how it stays invisible to RPG Maker.
...
- **Custom user interfaces**: a User Interfaces database section lays out menus, dialogs, and panels on a canvas at the game's screen size, with boxes, images, text, and buttons wired to common events, stock scenes, plugin commands, switches, variables, or other interfaces. The runtime draws them through the engine's own windows, and a Reactor-tab event command opens them. See the [design](docs/DESIGN-USER-INTERFACES.md) for what is built and what is next.
