...
- **The Forge, in-editor asset generators**:
  - **Animation Generator**: 75 procedural 2D animations across four categories with layered composition, per-layer keyframe timelines, a 3D shape pipeline, custom textures, and export to bake-ready sprite sheets or animated GIFs.
  - **Effekseer Animation Generator**: create native Effekseer particle effects (`.efkefc`) from 105 recipes across nine categories without the external Effekseer editor: 21 sci-fi interface instruments with user-typed text, physical battle hits, energy spells, elements, a custom-effect Composer, and more; wireframe or solid-textured rendering with custom texture upload; layers, keyframes with texture cross-fades, live in-editor preview through the game's own Effekseer runtime, and one-click export. The tracked suite validates generated format/model round trips, every recipe at default/extreme/swept values, composition, and real-WASM playback.
...
- **Map sections can be copied directly**: right-drag captures all four tile planes, shadows, and regions; left-click or drag stamps the exact six-plane patch with edge clipping, a live composite preview, and one Undo action per stroke. A synchronized A1 preference can pause editor water animation without changing playtests or deployments.
...
- **Untrusted project code stays off**: project JavaScript Character Generator parts require explicit per-project trust and that trust can be revoked; project PNG parts need no code trust. Project-authored content is escaped across affected privileged editor previews and forms.
- **Release artifacts are reproducible and authenticated**: publishable builds pin NW.js hashes, generate clean starters and source-bound manifests, require Windows signing and macOS signing/notarization, and publish inspected candidate bytes to GitHub and optional itch.io channels without rebuilding. Ordinary interactive builds can still use Latest Stable NW.js versions for development packages.
...
RPG Reactor runs as an NW.js desktop app. Source development and release tooling require Node.js 22 or newer. Source checkouts do not include NW.js platform binaries, `node_modules/`, build output, saves, or local project templates.
