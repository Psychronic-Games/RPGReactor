...
- **Palm, knuckle and fingertip markers (rig).** `ModelRigger` humanoid template: markers `palmL/R`, `knucklesL/R`, `fingersL/R` (defaults at(±.71,.41), at(±.73,.385), at(±.75,.36)); the runtime derives missing ones from the hand and forearm joints' own room positions (1.33 / 1.5 / 1.66 along elbow→wrist), never from the template's bone table, since a mapped file skeleton need not sit where the template does (that mismatch lost Jolt's pistol); `bones()` ends the Hand bone at the fingertips marker when present; `PREVIEW_LINKS` add wrist→palm→fingertips; `completeMarkers(saved, template, size)` overlays saved markers on defaults and derives missing palm/fingertips from the saved wrist and elbow (1.33 / 1.66 along elbow→wrist), and `Database3DEditor.enterRigMode` uses it instead of the old all-or-nothing default fallback (which would have discarded an old rig's placed markers). Runtime: `readModelRig` returns `markers`; `Reactor3D.attachRigHands(root, rig, bones)` (called from `applyModelRig` and `mapRigToSkeleton`) writes `hand.userData.__reactorPalm` / `__reactorFingers` as points in the hand joint's frame, from markers else derived from the bone heads; `BattleRoomView.attachmentQuaternion` uses `__reactorPalm` for the fist before any finger-root or forearm estimate. Four marker labels in 17 locales. Tests in `reactor-3d-rig.test.cjs`. The Demo rigs have no placed palm markers yet: Fleagus's palm is derived, and the owner can place it in the rig editor.
