...
- **Into the fist.** `attachmentPoint` for a hand attachment with no bone name adds .045 × model height along the forearm direction (`attachmentQuaternion().forearm`): the hand bone is the wrist, the fist is a little way on, and a handle belongs there (probe: grip .14 tiles past the wrist on Fleagus at every frame of the slash). The Fleagus model carries its own sheathed sword at the left hip, which is not the held one.
