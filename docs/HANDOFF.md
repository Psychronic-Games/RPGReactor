...
- **HUD names.** `P.installMogHudNames` wraps `Battle_Hud.prototype.refresh_name`: MZ starts a bare Bitmap in sans-serif (MV in the game font), so the name was the one HUD label in the wrong face; it now takes `$gameSystem.mainFontFace()` and steps its size down from the plugin's Name Font Size until the name fits the layout box (box width − 12 − outline). Verified: all seven names in rmmz-mainfont, the longest (Fleagus Gustafario, 135 px) inside the 160 px box.
- **Demo HUD.** MOG_BattleHud Custom Position 1–7 → x 370…1390 step 170, y 900 (160×170 boxes, 10 px from the bottom of 1920×1080); PSYCHRONIC_ATB-MZ bar centred: barX 335 (1250 wide), icon/skill/name X shifted +315. Verified in `scratchpad/demo-battle-hud-check.cjs` (Demo copy, room troop 1, seven actors, waits for the first command window): window at 1034,124 over the actor at 1134 with top 379; screenshot checked.
