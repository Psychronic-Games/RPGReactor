...
- **Editor.** Import Victor Notetags button and its 17-locale strings removed (`import-victor-motions.cjs` stays); the sequence header (name, Hits, Undo/Redo) heads the middle column. Vertical preview: Scene → Battle layout (+ Troop) in the cast row; remembered per project.
