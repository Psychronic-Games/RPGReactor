...
`0.98.2` below with the exact `editor/package.json` version. This validation
...
node -e "const p=require('./editor/package.json'); if(p.version!=='0.98.2') process.exit(1)"
```
...
`js/reactor_main.js` both advance to `0.98.2`, while `js/reactor_plugins.js`
...
node editor/build-scripts/release-editor.cjs \
  --target linux --mode candidate --version 0.98.2 \
...
`dist-editor/releases/v0.98.2/<target>/` directory and an
...
and test count, creates a release commit when needed, creates `v0.98.2`, and
...
gh release view v0.98.2
test "$(git rev-parse v0.98.2^{commit})" = "$(git rev-parse origin/main)"
```
...
- `version` is `0.98.2`, `nwjsVersion` is `0.107.0`, and `sourceCommit` is the tag commit.
...
2. Confirm About/package version is `0.98.2`.
...
**Publish Release** with version `0.98.2` before announcing the release. That
...
```bash
git push origin :refs/tags/v0.98.2
git tag -d v0.98.2
```
