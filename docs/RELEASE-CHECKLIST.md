...
`0.98.3` below with the exact `editor/package.json` version. This validation
...
node -e "const p=require('./editor/package.json'); if(p.version!=='0.98.3') process.exit(1)"
```
...
`js/reactor_main.js` both advance to `0.98.3`, while `js/reactor_plugins.js`
...
node editor/build-scripts/release-editor.cjs \
  --target linux --mode candidate --version 0.98.3 \
...
`dist-editor/releases/v0.98.3/<target>/` directory and an
...
and test count, creates a release commit when needed, creates `v0.98.3`, and
...
gh release view v0.98.3
test "$(git rev-parse v0.98.3^{commit})" = "$(git rev-parse origin/main)"
```
...
- `version` is `0.98.3`, `nwjsVersion` is `0.107.0`, and `sourceCommit` is the tag commit.
...
2. Confirm About/package version is `0.98.3`.
...
**Publish Release** with version `0.98.3` before announcing the release. That
...
```bash
git push origin :refs/tags/v0.98.3
git tag -d v0.98.3
```
