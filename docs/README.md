...
- [Custom user interfaces](DESIGN-USER-INTERFACES.md): the positions behind the User Interfaces section (records in their own data file, engine windows rather than the DOM, invisible to RPG Maker), the node and action model, and the phasing: the node editor, capture from game, and stock baselines are built; List and Gauge nodes, overlays, and capture-to-nodes are next.
