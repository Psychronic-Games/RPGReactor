...
// Dirs inside the player-facing template/Demo/ to exclude
const TEMPLATE_EXCLUDE = new Set([
    'Backup', 'Screenshots', 'REACTOR_CORE_DUMP_MIDDEV', 'RMMZ_Corescript', 'PIXI5', 'PIXI8',
]);
const TEMPLATE_EXCLUDE_FILES = new Set(['nul']);
...
function copyTemplateFiltered(src, dest) {
    if (!fs.existsSync(src)) return;
    fs.mkdirSync(dest, { recursive: true });
    const entries = fs.readdirSync(src, { withFileTypes: true });
    for (const entry of entries) {
        if (TEMPLATE_EXCLUDE.has(entry.name)) {
            logDim(`  [skip] ${entry.name}/`);
            continue;
        }
        if (!entry.isDirectory() && TEMPLATE_EXCLUDE_FILES.has(entry.name)) {
            logDim(`  [skip] ${entry.name}`);
            continue;
        }
        const s = path.join(src, entry.name);
...
(async () => {
    ...
    // Copy template/Demo/ with exclusions. Barebones and Complex are development templates.
    const templateSrc = path.join(appRoot, 'template', 'Demo');
    if (fs.existsSync(templateSrc)) {
        fs.mkdirSync(path.join(stageRoot, 'template'), { recursive: true });
        copyTemplateFiltered(templateSrc, path.join(stageRoot, 'template', 'Demo'));
    }
